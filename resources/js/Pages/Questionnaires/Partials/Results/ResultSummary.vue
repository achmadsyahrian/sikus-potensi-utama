...
const stats = computed(() => {
    let filtered = props.summaryStats || [];
    ...
    if (selectedRole.value !== 'all') {
        if (['alumni', 'mitra', 'pengguna_lulusan'].includes(selectedRole.value)) {
            filtered = filtered.filter(s => s.external_role === selectedRole.value);
        } else if (selectedRole.value === 'external_all') {
            filtered = filtered.filter(s => s.external_role !== null);
        } else {
            filtered = filtered.filter(s => s.role_id == selectedRole.value);
        }
    }
    ...
    filtered.forEach(stat => {
        const opt = props.questionnaire.options.find(o => o.option_value == stat.answer_value);
        if (opt) {
            counts[opt.option_text] += Number(stat.total);
            totalAnswers += Number(stat.total);
        }
    });
    ...
    return {
        ...
        total: totalAnswers,
        percentages: Object.values(counts).map(v => totalAnswers > 0 ? ((v / totalAnswers) * 100).toFixed(1) : 0)
    };
});
...
const renderChart = () => {
    const chartEl = document.getElementById('global-distribution-chart');
    if (!chartEl) return;
...
watch([selectedCategory, selectedRole, chartType, stats], () => {
    setTimeout(() => {
        renderChart();
    }, 50);
}, { deep: true });
...
<template>
    <div class="card card-md">
        ...
        <div class="card-body border-top border-bottom bg-light-lt py-4">
            <div id="global-distribution-chart" class="text-dark" v-show="stats.total > 0"></div>
